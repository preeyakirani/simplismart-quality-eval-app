...
def file_transcribe_simplismart(audio_file, 
    ...
    if (simplismart_res == "ERROR"):
        with open("cached_examples/cache_simplismart", "rb") as cache_file:
            data = pickle.load(cache_file)
            if audio_file.split("/")[-1] in data.keys():
                simplismart_prediction = data[audio_file.split("/")[-1]]["prediction"]
                simplismart_res = data[audio_file.split("/")[-1]]["response"]
                time_taken = data[audio_file.split("/")[-1]]["time_taken"]
                gr.Warning(message="Something went wrong with Simplismart. Using cached example.")
            else:
                raise gr.Error(message="Something went wrong with Simplismart. If issue persists, contact admin.")
    else:
...
def file_transcribe_deepgram(audio_file, actual_tr, task, language):
    if (task == "translate"):
        return [None, "{TRANSLATION NOT SUPPORTED NATIVELY}", "NA", 0.00,
                gr.Button(value="Submit", variant="primary", interactive=False)]
    else:
        deepgram_res, time_taken = get_transcription_deepgram(audio_file, get_lang_code(language))

        if (deepgram_res == "ERROR"):
            with open("cached_examples/cache_deepgram", "rb") as cache_file:
                data = pickle.load(cache_file)
                if audio_file.split("/")[-1] in data.keys():
                    deepgram_prediction = data[audio_file.split("/")[-1]]["prediction"]
                    deepgram_res = data[audio_file.split("/")[-1]]["response"]
                    time_taken = data[audio_file.split("/")[-1]]["time_taken"]
                    gr.Warning(message="Something went wrong with Deepgram. Using cached example.")
                else:
                    raise gr.Error(message="Something went wrong with Deepgram. If issue persists, contact admin.")
        else:
...
def file_transcribe_assemblyai(audio_file, actual_tr, task, language):
    if (task == "translate"):
        ...
        assembly_res = "ERROR"
        if (assembly_res == "ERROR"):
            with open("cached_examples/cache_assembly", "rb") as cache_file:
                data = pickle.load(cache_file)
                if audio_file.split("/")[-1] in data.keys():
                    assembly_prediction = data[audio_file.split("/")[-1]]["prediction"]
                    response_words = data[audio_file.split("/")[-1]]["response"]
                    time_taken = data[audio_file.split("/")[-1]]["time_taken"]
                    gr.Warning(message="Something went wrong with Assembly AI. Using cached example.")
                else:
                    raise gr.Error(message="Something went wrong with Assembly AI. If issue persists, contact admin.")
        else:
...
def file_transcribe_rev_ai(audio_file, actual_tr, task, language):
    ...
    if (rev_res_text == "ERROR"):
        with open("cached_examples/cache_rev", "rb") as cache_file:
            data = pickle.load(cache_file)
            if audio_file.split("/")[-1] in data.keys():
                rev_prediction = data[audio_file.split("/")[-1]]["prediction"]
                rev_res = data[audio_file.split("/")[-1]]["response"]
                time_taken = data[audio_file.split("/")[-1]]["time_taken"]
                gr.Warning(message="Something went wrong with Rev AI. Using cached example.")
            else:
                raise gr.Error(message="Something went wrong with Rev AI. If issue persists, contact admin.")
    else:
...
def file_transcribe_whisper(audio_file, actual_tr, task, language):
    ...
    if (whisper_res == "ERROR"):
        with open("cached_examples/cache_whisper", "rb") as cache_file:
            data = pickle.load(cache_file)
            if audio_file.split("/")[-1] in data.keys():
                whisper_prediction = data[audio_file.split("/")[-1]]["prediction"]
                whisper_res_words = data[audio_file.split("/")[-1]]["response"]
                time_taken = data[audio_file.split("/")[-1]]["time_taken"]
                gr.Warning(message="Something went wrong with Open AI Whisper. Using cached example.")
            else:
                raise gr.Error(message="Something went wrong with Open AI Whisper. If issue persists, contact admin.")
    else:
        whisper_prediction = whisper_res.text
        whisper_res_words = whisper_res.words
    ...
    if (task == "transcribe"):
        return [whisper_prediction, whisper_res_words, colored_whisper_prediction, whisper_wer, time_taken,
                gr.Button(value="Submit", variant="primary", interactive=False)]
    else:
        return [whisper_prediction, None, colored_whisper_prediction, whisper_wer, time_taken,
                gr.Button(value="Submit", variant="primary", interactive=False)]
...
def file_transcribe_aws(audio_file, actual_tr, task, language):
    if (task == "translate"):
        ...
        if (aws_res == "ERROR"):
            with open("cached_examples/cache_aws", "rb") as cache_file:
                data = pickle.load(cache_file)
                if audio_file.split("/")[-1] in data.keys():
                    aws_prediction = data[audio_file.split("/")[-1]]["prediction"]
                    aws_res = data[audio_file.split("/")[-1]]["response"]
                    time_taken = data[audio_file.split("/")[-1]]["time_taken"]
                    gr.Warning(message="Something went wrong with AWS Transcribe. Using cached example.")
                else:
                    raise gr.Error(message="Something went wrong with AWS Transcribe. If issue persists, contact admin.")
        else:
